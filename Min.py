...
cl = LINETCR.LINE()
#cl.login(qr=True)
cl.login(token="EqVWhlFiortSQ0BcmDw5.T7KcSucUsxqj+nFh5yBIHq.MpD5xbIdCIwtdAXKako+VZmOknwvggugNSwp1LoY8Hc=") 
cl.loginResult()
...
autoleaveroom = []
targets = []
Bots = [mid,"ueb80efb7a8eb143232b3c188a5a64f5e"]
...
#-------------ขโมย---------------
            elif msg.text in ["tag","Tag","รักนะ"]:
                 group = cl.getGroup(msg.to)
                 nama = [contact.mid for contact in group.members]
                 nm1, nm2, nm3, nm4, nm5, jml = [], [], [], [], [], len(nama)
                 if jml <= 100:
